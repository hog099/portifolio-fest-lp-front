...
export const SectionInLive: React.FC = () => {
  ...
  // const [liveSelected, setLiveSelected] = useState<any>(null);
      ...
      {!loading && (
        <>
          ...
          <div className='w-[100%] flex flex-col items-center mb-12'>
            <h3 className='text-white text-3xl my-2 text-center'>Live 18-08-2024 (selecionada)</h3>
            <div
            className={styles['content-video-selected']}
              dangerouslySetInnerHTML={{
                __html:
                  '<iframe width="560" height="315" src="https://www.youtube.com/embed/7X8II6J-6mU?si=4bD-n5q2siRZI9PT" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>',
              }}
            />
          </div>
          <div className={styles['content-about']}>
            {[...Array(4)].map((_, index) => (
              <button key={index} className={styles['button-thumb']} onClick={() => alert('Implementar Selecao')}>
                <div
                ...
                />
                <p className='text-white text-md my-2'>Live 11-08-2024</p>
              </button>
            ))}
          </div>
        </>
